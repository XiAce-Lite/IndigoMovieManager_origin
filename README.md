...
- 任意サムネイル作成機能あり
- プレビュー小窓あり
- ライブラリ全体を対象に、スキン切替（WPF/WB・スキン名変更）時に未作成サムネの自動生成を行う
  - 件数が多いときはステータスバーに「サムネイル確認中 (スキン名)」を先に表示する
- 検索が空のときはライブラリ全体を、キーワード検索中は表示中の絞り込み結果だけを生成対象にする
  - 検索キーワードを変更すると、進行中の一覧サムネ生成は止まり、表示中の絞り込み結果（未作成のみ）向けに再開する
  - 検索をクリアして全件表示に戻すと、ライブラリ全体を対象に再開する
  - `{::error}` で0件になった場合も、進行中の生成は止まる（対象0件のため新規開始はしない）
  - DB の再読込や一覧の強制更新が必要なときはヘッダーの **更新** を押す
...
- 個別設定で除外拡張子を指定可能
  ...
  - 動画ファイルのフォルダ内に画像が入ったZipファイルが混在するケースで、Zipファイルを除外したいときなどを想定
