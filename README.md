...
- 任意サムネイル作成機能あり
- プレビュー小窓あり
- 一覧に表示中のレコード向けに、スキン切替や検索条件の変更時に未作成サムネの自動生成を行う
  - 検索をクリアして全件表示に戻した場合も、表示対象が変わるため生成対象を切り替える
  - レイアウトの重いスキン（例: WideGridInfo）では、切替直後に生成が始まらないことがある。そのときはヘッダーの **更新** を押す
